...
struct DonutGallery: View {
    ...
    var body: some View {
        ...
        .navigationDestination(for: Donut.ID.self) { donutID in
            DonutEditor(donut: model.donutBinding(id: donutID), cartModel: cartModel)
        }
        .navigationDestination(for: String.self) { _ in
            DonutEditor(donut: $model.newDonut, cartModel: cartModel)
        }
    }
...
struct DonutBakery_Previews: PreviewProvider {
    struct Preview: View {
        ...
        var body: some View {
            DonutGallery(model: model, cartModel: CartModel())
        }
    }
