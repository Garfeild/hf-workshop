...
public struct OrderGenerator {
    ...
    public init(knownDonuts: [Donut] = Donut.all) {
        self.knownDonuts = knownDonuts
    }

    public func todaysOrders(totalOrders: Int = 24) -> [Order] {
    ...
    public func generateOrder(number: Int, date: Date) -> Order {
    ...
    public func generateOrder(number: Int, date: Date, donuts: [Donut], sales: [Donut.ID: Int]) -> Order {
